...
- Make sure to run the `format` and `build` scripts after updating the template to verify it still works (`build` also runs oxlint). From the repo root you can also run `bun run build` to check every template at once.
