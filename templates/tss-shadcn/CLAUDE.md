...
- `src/routes` contains file-based routes, powered by TanStack Router:
  - `__root.tsx` — the root route; defines the document shell (`<html>`/`<head>`/`<body>`) that wraps every route via `<Outlet />`.
  - `index.tsx` — the `/` route.
  - Dynamic segments are prefixed with `$` (e.g. `$templateName.tsx` matches `/:templateName`).
  - A directory groups routes under a shared path segment; its `route.tsx` is the parent layout for its children, e.g.:
    ```
    posts/
    ...
    ```
  - A directory prefixed with `_` is a pathless layout route — its `route.tsx` wraps its children without adding a path segment, e.g.:
    ```
    _account/
    ...
    ```
  - `src/routeTree.gen.ts` is generated by the TanStack Router plugin on dev/build — never edit it directly.
