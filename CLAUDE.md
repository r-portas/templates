...
- The repo root is a [Bun workspace](https://bun.sh/docs/install/workspaces) (see @package.json), with each template and `./docs` as a workspace package. Use the root scripts to operate across all of them at once:
  - `bun run update`: update every workspace package's dependencies to the latest versions
  - `bun run lint`: lint every workspace package
  - `bun run build`: build every workspace package, verifying it still works
...
When making a change to a template, consider:
...
- Is the template added to @.github/workflows/build.yml?
- Is the template's lineage recorded in the Lineage section above?
