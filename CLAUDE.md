...
- Templates build on each other, so changes should flow downstream through this lineage:
  ...
  - `tss-tw` builds on `tss`, adding Tailwind CSS
  - `tss-shadcn` builds on `tss-tw`, adding shadcn/ui, and is the end of the lineage
  - `./docs` (the documentation site, outside `./templates`) is not part of this lineage. It is a standalone TanStack Start site, maintained directly, and follows the conventions of the `tss-shadcn` template.
...
- When adding a new addon consider:
  ...
  - Does the addon target the conventions used by the `tss-shadcn` template
